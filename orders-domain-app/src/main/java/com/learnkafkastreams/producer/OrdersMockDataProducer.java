...
import com.learnkafkastreams.util.ProducerUtil;
...
import static com.learnkafkastreams.util.ProducerUtil.publishMessageSync;
...
//        publishExpiredRecords(objectMapper);
        ...
        orders
                .forEach(order -> {
                    try {
                        ...
                        var recordMetaData = publishMessageSync(ORDERS, null, ordersJSON);
    ...
    private static void publishOrdersToTestGrace(ObjectMapper objectMapper, List<Order> orders) {
        ...
        orders
                .forEach(order -> {
                    try {
                        var ordersJSON = objectMapper.writeValueAsString(order);
                        var recordMetaData = publishMessageSync(ProducerUtil.ORDERS, order.orderId() + "", ordersJSON);
                        log.info("Published the order message : {} ", recordMetaData);
                    } catch (JsonProcessingException e) {
                        log.error("JsonProcessingException : {} ", e.getMessage(), e);
                        throw new RuntimeException(e);
    ...
    private static List<Order> buildOrdersToTestGrace() {
        ...
        var order1 = new Order(12345, "store_1234",
                ...
                OrderType.GENERAL,
                orderItems,
                LocalDateTime.parse("2025-11-28T08:42:58")
                //LocalDateTime.now(ZoneId.of("UTC"))
        );
